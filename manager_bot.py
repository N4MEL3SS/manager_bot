...
• 🎫 Список тикетов - показать неотвеченные вопросы
...
⏰ Обновлено: {datetime.now(pytz.timezone(config.TIMEZONE)).strftime('%H:%M %d.%m.%Y')}
        ...
        # Добавляем статистику по менеджерам
        if managers:
            stats_text += "\n📋 Активные менеджеры:\n"
            for i, manager in enumerate(managers, 1):
                manager_stats = await db.get_manager_stats(manager.chat_id)
                last_activity = manager_stats['last_activity'].strftime('%d.%m.%Y %H:%M') if manager_stats['last_activity'] else "нет активности"
                stats_text += f"{i}. {manager.nickname} - {manager_stats['total_answered']} ответов\n"
    ...
    try:
        ...
        if existing_manager and existing_manager.is_active:
            await message.answer(
                f"❌ Пользователь с chat_id {chat_id} уже является менеджером",
                reply_markup=get_admin_keyboard()
            )
            await state.clear()
            return
...
Теперь пользователь имеет доступ к боту менеджера.
        ...
        try:
            ...
            await manager_bot.send_message(
                chat_id=chat_id,
                text=f"🎉 Вас добавили как менеджера поддержки!\n\n"
...
async def list_managers(callback: CallbackQuery):
    ...
    try:
        ...
        for i, manager in enumerate(managers, 1):
            ...
            last_activity = stats['last_activity'].strftime('%d.%m.%Y %H:%M') if stats['last_activity'] else "Нет активности"
...
async def remove_manager_start(callback: CallbackQuery):
    ...
    try:
        ...
        if not managers:
            await callback.message.edit_text(
                "📭 Нет активных менеджеров для удаления",
                reply_markup=get_admin_keyboard()
            )
            await callback.answer()
            return
...
async def remove_manager_execute(callback: CallbackQuery):
    ...
    try:
        ...
        if success:
            await callback.message.edit_text(
                f"✅ Менеджер {manager.nickname} успешно удален",
                reply_markup=get_admin_keyboard()
            )
            ...
            try:
                ...
                await manager_bot.send_message(
                    ...
                    text="❌ Ваш доступ к боту менеджера был отозван."
                )
                await manager_bot.session.close()
            except Exception as e:
...
🎫 Список тикетов - показать все неотвеченные вопросы
📊 Статистика - показать статистику тикетов и менеджеров
👥 Управление менеджерами - управление доступом (только для админов)
...
4. Ответ автоматически отправится клиенту
...
async def handle_manager_message(message: Message, state: FSMContext):
    ...
    if current_state == ManagerStates.waiting_for_ticket_answer:
        ...
        try:
            ...
            client_bot = Bot(token=config.CLIENT_BOT_TOKEN)
            answer_text = f"""
📨 Ответ от поддержки на ваш вопрос (#{ticket_id}):
...
Спасибо, что обратились к нам! 😊
            ...
            # Уведомляем других менеджеров о том, что тикет взят в работу
            asyncio.create_task(
                notification_manager.notify_ticket_answered(ticket, message.chat.id)
            )
...
async def close_ticket(callback: CallbackQuery):
    ...
    try:
        ticket = await db.answer_ticket(
            ticket_id=ticket_id,
            answer="Тикет закрыт без ответа",
            manager_chat_id=callback.message.chat.id
        )
        
        await callback.message.edit_text(
            f"✅ Тикет #{ticket_id} закрыт без ответа",
            reply_markup=None
        )
        await callback.answer()
